...
/*class Solution {
    public List<List<Integer>> zigzagLevelOrder(TreeNode root) 
    {
        ...
        for (int i = 0; i<ans.size(); i++)
        {
            if (i%2 == 1)
            {
                List<Integer> lstNormal = ans.get(i);
                List<Integer> lstReverse = new LinkedList<Integer>();
                for (int j = lstNormal.size() -1; j>=0; j--)
                {
                    lstReverse.add(lstNormal.get(j));
                }
                ans.set(i, lstReverse);
            }
        }
        return ans;
    }
